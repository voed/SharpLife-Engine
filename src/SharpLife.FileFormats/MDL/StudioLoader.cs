    ...
    public sealed class StudioLoader
    {
        ...
        private unsafe List<Texture> ReadTextures(in MainHeader header)
        {
            ...
            for (var i = 0; i < header.NumTextures; ++i)
            {
                ...
                foreach (var j in Enumerable.Range(0, WADConstants.NumPaletteColors))
                {
                    var paletteData = _reader.ReadBytes(WADConstants.NumPaletteComponents * WADConstants.PaletteComponentSizeInBytes);
